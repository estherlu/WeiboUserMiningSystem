...
namespace WbTracker
{
	...
	public class DatabaseConnect: System.IDisposable 
	{
        ...
        public static FileStream ConfigFile;
		public DatabaseConnect()
		{
           
            try
            {
                ConfigFile = new FileStream("config.txt", FileMode.Open, FileAccess.Read);
                StreamReader ConfigReader = new StreamReader(ConfigFile);
                ConfigReader.BaseStream.Seek(0, SeekOrigin.Begin);
                source = ConfigReader.ReadLine(); 
            }
            catch
            {
                MessageBox.Show("配置文件不存在，请检查。", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Application.Exit();
            }
...
//			conn=null;
		...
		private void dataConnect()
		{
            if (source != null)
            {
                ...
                catch (Exception er)
                {
                    ...
                    MessageBox.Show("配置文件或者数据库错误，请检查。详细信息：" + er.Message, "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    Application.Exit();
                }
            }
            else
            {
                MessageBox.Show("配置文件信息缺失，请检查。", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Application.Exit();
            }
		}
		/// <summary>
		...
		public ArrayList getData(string SQL)
		{
			...
			ArrayList data=new ArrayList ();
                ...
                cmd.CommandTimeout = cmd.CommandTimeout + 1000;
		...
		private void dataOperater(string strSQL)
		{
			...
			{
                if (!e.Message.StartsWith("违反了 PRIMARY KEY 约束"))
                {
                    cmd.Dispose();
                    this.closeConnect();
		...
		/// <returns></returns>
		public static string ReplaceDYH(string str)
		{
			if (str.Trim ()=="") return "";
			str=str.Replace ("'","＇");
            str = str.Replace(",", "，");
